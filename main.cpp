// for reasons to enforce the nvidia driver to always make use of the dGpu for
...
// This tells the NVIDIA driver to always use the discrete GPU for this
// application
extern "C"
{
...
// this alignas(16) is to make sure that the offset of these members are aligned
// with shader code definition
...
class HelloTriangleApplication
{
  ...
  private:
    ...
    void initWindow()
    {
        ...
        // Create the window. We pass 'this' as the last parameter so we can
        // retrieve it in WM_CREATE
    ...
    bool isDeviceSuitable(VkPhysicalDevice device)
    {
        ...
        return indices.isComplete() && extensionsSupported && swapChainAdequate && isDiscreteGpu(device);
    }
    ...
    void createSwapChain()
    {
        ...
        std::cout << "Max Array Layers: " << swapChainSupport.capabilities.maxImageArrayLayers << std::endl;

        // --- ADD THIS LINE ---
        std::cout << "Supported Usage Flags: " << swapChainSupport.capabilities.supportedUsageFlags << std::endl;
        ...
        if (indices.graphicsFamily != indices.presentFamily)
        {
            ...
            std::cout << "Swap Chain Create: Using CONCURRENT sharing. G=" << indices.graphicsFamily.value()
                      << ", P=" << indices.presentFamily.value() << std::endl; // Log
        }
        else
        {
            ...
            std::cout << "Swap Chain Create: Using EXCLUSIVE sharing. G=P=" << indices.graphicsFamily.value()
                      << std::endl; // Log
        }
        ...
        // --- MORE LOGGING ---
        std::cout << "Swap Chain Create: Format=" << surfaceFormat.format << ", ColorSpace=" << surfaceFormat.colorSpace
        ...
        for (int i = 0; i < MAX_RETRIES; ++i)
        {
            ...
            // If successful or the error is not the one we can recover from, break
            // the loop
            ...
            std::cout << "Warning: vkCreateSwapchainKHR failed with "
                         "VK_ERROR_NATIVE_WINDOW_IN_USE_KHR. Retrying ("
    ...
    void cleanup()
    {
        ...
        // no need to destroy descriptorSets; they will get automatically freed when
        // the descriptorPool is destroyed
    ...
    void createInstance()
    {
        ...
        //     std:: cout << "extensions required does not match extensions
        //     supported by vulkan"; return;
        // }
    ...
    void createGraphicsPipeline()
    {
        ...
        VkPipelineShaderStageCreateInfo shaderStages[] = {vertShaderCreateInfo, fragShaderStageInfo};
    ...
    SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device)
    {
        ...
        // --- ADD THIS LOGGING ---
        std::cout << "Swap Chain Query: Found " << formatCount << " surface format(s)." << std::endl;
        ...
        std::cout << "Swap Chain Query: Found " << presentModeCount << " present modes." << std::endl; // Log
    ...
    QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device)
    {
        ...
        for (const auto &queueFamily : queueFamilies)
        {
            ...
            std::cout << "Graphics(" << ((queueFamily.queueFlags & VK_QUEUE_GRAPHICS_BIT) ? "Yes" : "No") << "), ";
            ...
            if (indices.graphicsFamily.has_value() && indices.presentFamily.has_value() &&
                indices.transferFamily.has_value() &&
                !(queueFamilies[indices.transferFamily.value()].queueFlags & VK_QUEUE_GRAPHICS_BIT))
            {
                break;
            }
            i++;
    ...
    void createCommandPools()
    {
        ...
        if (vkCreateCommandPool(device, &graphicsQueuePoolInfo, nullptr, &graphicsQueueCommandPool) != VK_SUCCESS &&
            vkCreateCommandPool(device, &transferQueuePoolInfo, nullptr, &transferQueueCommandPool) != VK_SUCCESS)
        {
    ...
    void createCommandBuffers()
    {
        ...
        graphicsQueueCommandBufferAllocInfo.commandBufferCount = (uint32_t)graphicsQueueCommandBuffers.size();
        ...
        transferQueueCommandBufferAllocInfo.commandBufferCount = (uint32_t)transferQueueCommandBuffers.size();

        if (vkAllocateCommandBuffers(device, &graphicsQueueCommandBufferAllocInfo,
                                     graphicsQueueCommandBuffers.data()) != VK_SUCCESS &&
            vkAllocateCommandBuffers(device, &transferQueueCommandBufferAllocInfo, transferQueueCommandBuffers.data()))
        {
    ...
    void updateUniformBuffer(uint32_t currentImage)
    {
        ...
        // glm library was made for openGL which makes the y component of the
        // projection matrix inverted; in our use leaving it as is would produce
        // inverted rendering thus we invert it later to the calculation of the
        // projection matrix
        ubo.proj[1][1] *= -1;
